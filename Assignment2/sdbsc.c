...
int get_student(int fd, int id, student_t *s) {
    off_t lseek_result = lseek(fd, (id) * STUDENT_RECORD_SIZE, SEEK_SET);
    if (lseek_result < 0) {
        return ERR_DB_FILE; 
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t tempstu = {0};
    if (get_student(fd, id, &tempstu) != SRCH_NOT_FOUND) {
...
int count_db_records(int fd){
    int i = 0;
    int end = STUDENT_RECORD_SIZE;
    int cnt = 0;
    student_t temp = {0};
    while(end == STUDENT_RECORD_SIZE) {
        off_t lseek_result = lseek(fd, i * STUDENT_RECORD_SIZE, SEEK_SET);
        if (lseek_result < 0) {
            return ERR_DB_FILE; 
        }
        i++;
        end = read(fd, &temp, STUDENT_RECORD_SIZE);
        if (end < 0) {
            printf(M_ERR_DB_READ);
            return ERR_DB_FILE;
        }
        if(!(memcmp(&temp, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) && (end == STUDENT_RECORD_SIZE)) {
            cnt++;
        }
    }
    if (cnt == 0) {
...
int print_db(int fd){
    ...
    while(end == STUDENT_RECORD_SIZE) {
        ...
        calculated_gpa_from_student = temp.gpa/100.0;
        if((memcmp(&temp, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0 && (end == STUDENT_RECORD_SIZE))) {
            cnt++;
            if(empty == 0 && cnt == 1) {
            ...
            printf(STUDENT_PRINT_FMT_STRING, temp.id, temp.fname, temp.lname, calculated_gpa_from_student);
        }
    }
    if (cnt == 0) {
        printf(M_DB_EMPTY);
