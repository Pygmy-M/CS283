...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        if (!(strcmp(cmd_list.commands[0].argv[0], "") == 0)) {
            rc = rsh_execute_pipeline(cli_socket, &cmd_list);

            switch (rc)
            {
            case -100:
                strcpy(io_buff, EXIT_CMD);
                printf(RCMD_MSG_CLIENT_EXITED);
            ...
            case -300:
                char temp[20];
                sprintf(temp, "%d", last_rc);
                strcpy(io_buff, temp);
                printf(RCMD_MSG_SVR_RC_CMD, last_rc);
                send_message_string(cli_socket, io_buff);
            default:
                if (rc != 0) {
                    strcpy(io_buff, CMD_ERR_RDSH_EXEC);
                    send_message_string(cli_socket, io_buff);
                }
            }     
        } else {
            strcpy(io_buff, CMD_WARN_NO_CMD);
            send_message_string(cli_socket, io_buff);
        }
        ...
        //  - etc.
        if (cmd_rc != 0) {
            strcpy(io_buff, CMD_ERR_RDSH_COMM);
            send_message_string(cli_socket, io_buff);
        } else if (rc != 0) {
            strcpy(io_buff, CMD_ERR_RDSH_EXEC);
            send_message_string(cli_socket, io_buff);
...
//Adapted from dshlib.c for easy processing use and readability
int svr_build_cmd_list(char *cmd_line, command_list_t *buff) {
...
int send_message_string(int cli_socket, char *buff){
    ...
    while (total_sent < len) {
        sent_len = send(cli_socket, buff + total_sent, len - total_sent, 0);
        if (sent_len < 0) {
            // Handle error: Send error message and return
            send(cli_socket, CMD_ERR_RDSH_COMM, sizeof(CMD_ERR_RDSH_COMM), 0);
            return ERR_RDSH_COMMUNICATION;
        }
        total_sent += sent_len;
    }
...
int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int l = 0; l < clist->num; l++) {
        for (int j = 0; j < clist->commands[l].argc; j++) {
            free(clist->commands[l].argv[j]);
            clist->commands[l].argv[j] = NULL;
        }
    }
